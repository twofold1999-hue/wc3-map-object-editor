...
* **Four first-pass map-level script modules built on targeted `war3map.j` rewriting**
...
* four map-level gameplay script modules:
* kill reward / hero growth
* starting resources
* starting hero power
* player-specific starting settings

This milestone reflects the point where the project moved beyond a pure round-trip prototype and into a genuinely multi-module Warcraft III gameplay modding toolbox.

---

## Next milestone direction

**v0.8 - reusable script transformations and deeper gameplay modules**

The next phase focuses on two parallel directions:

1. continuing semantic refinement of the internal object model
2. expanding gameplay-level tooling beyond the current four script modules
...
On the gameplay side, the project now has several stable trigger-focused modules and can begin either:

* deepening current preset layers
* adding a fifth clearly scoped gameplay module
* extracting more reusable script transformation helpers across modules
...
The project now includes four focused `war3map.j` gameplay modules.
...
* `--player` remains required
* explicit numeric flags override preset values
* unified starting resource blocks remain the defaults
* player-specific blocks act as overrides for individual players
...
<pre class="overflow-visible! px-0!" data-start="16889" data-end="17201"><div class="relative w-full mt-4 mb-1"><div class=""><div class="relative"><div class="h-full min-h-0 min-w-0"><div class="h-full min-h-0 min-w-0"><div class="border border-token-border-light border-radius-3xl corner-superellipse/1.1 rounded-3xl"><div class="h-full w-full border-radius-3xl bg-token-bg-elevated-secondary corner-superellipse/1.1 overflow-clip rounded-3xl lxnfua_clipPathFallback"><div class="pointer-events-none absolute inset-x-4 top-12 bottom-4"><div class="pointer-events-none sticky z-40 shrink-0 z-1!"><div class="sticky bg-token-border-light"></div></div></div><div class="relative"><div class=""><div class="relative z-0 flex max-w-full"><div id="code-block-viewer" dir="ltr" class="q9tKkq_viewer cm-editor z-10 light:cm-light dark:cm-light flex h-full w-full flex-col items-stretch ͼ5 ͼj"><div class="cm-scroller"><div class="cm-content q9tKkq_readonly"><span>    // wc3moe-player-starting-gold-begin</span><br/><span>    call SetPlayerState(Player(1), PLAYER_STATE_RESOURCE_GOLD, 200)</span><br/><span>    // wc3moe-player-starting-gold-end</span><br/><span>    // wc3moe-player-starting-lumber-begin</span><br/><span>    call SetPlayerState(Player(1), PLAYER_STATE_RESOURCE_LUMBER, 25)</span><br/><span>    // wc3moe-player-starting-lumber-end</span></div></div></div></div></div></div></div></div></div></div><div class=""><div class=""></div></div></div></div></div></pre>
